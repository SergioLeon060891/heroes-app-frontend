...
export const HeroGridCard = ({ hero }: Props) => {
  ...
  return (
    <>
      <Card
        key={hero.id}
        className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-to-br from-white to-gray-50"
      >
        <div className="relative h-64">
          <img
            src={hero.image}
            alt={hero.alias}
            className="object-cover transition-all duration-500 group-hover:scale-110 absolute top[-30px] w-full h-96.25 cursor-pointer"
            onClick={handleClick}
          />
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {hero.powers.slice(0, 3).map((power, pwrIndex) => (
                <Badge key={pwrIndex} variant="outline" className="text-xs">
                  {power}
                </Badge>
              ))}
